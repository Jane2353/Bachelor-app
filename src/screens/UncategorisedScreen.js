      ...
      try {
        ...
        if (data) {
          ...
          const uniqueCategories = [
            ...new Set(parsed.map(exp => exp.category).filter(Boolean).filter(cat => cat !== 'Uncategorized')),
          ];
  ...
  const handleCategorySelect = (category) => {
    if (selectedExpense) {
      ...
      const updatedAllExpenses = allExpenses.map((expense) =>
        expense.date === selectedExpense.date && expense.amount === selectedExpense.amount
          ? { ...expense, category }
          : expense
      );
